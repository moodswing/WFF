...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  exclude-result-prefixes="jscript">
  ...
  <xsl:template name="controlForm">
    ...
    <xsl:choose>
      <xsl:when test="$node/type='textbox'">
        <input type="textbox">
          <xsl:attribute name="id"><xsl:value-of select="$node/controlId"/></xsl:attribute>
          <xsl:attribute name="name"><xsl:value-of select="$node/controlId"/></xsl:attribute>
          <xsl:if test="$node/format != ''">
            <xsl:attribute name="type"><xsl:value-of select="$node/format"/></xsl:attribute>
          </xsl:if>
          <xsl:if test="$node/value != ''">
            <xsl:attribute name="value"><xsl:value-of select="$node/value"/></xsl:attribute>
          </xsl:if>
          ...
          <xsl:if test="$node/visibleWhen">
            <xsl:attribute name="data-visibleWhen"><xsl:value-of select="$node/visibleWhen"/></xsl:attribute>
          </xsl:if>
        </input>
      </xsl:when>
      ...
      <xsl:when test="$node/type='label'">
            ...
            <xsl:when test="$node/dataOrigin != ''">
              <xsl:choose>
                <xsl:when test="$node/dataOrigin='initialInput'">
                  <xsl:attribute name="value"><xsl:value-of select="//body/initialInput/*[local-name() = $node/value]"/></xsl:attribute>
                </xsl:when>
              </xsl:choose>
            </xsl:when>
            <xsl:otherwise>
        ...
        <xsl:if test="$node/alternativeValue = 'true'">
          <input type="hidden">
            <xsl:attribute name="name"><xsl:value-of select="$node/controlId"/><xsl:value-of select="$node/alternativeSuffix"/></xsl:attribute>
            <xsl:attribute name="id"><xsl:value-of select="$node/controlId"/><xsl:value-of select="$node/alternativeSuffix"/></xsl:attribute>
          </input>
        </xsl:if>
      </xsl:when>
              ...
              <label class="radiobutton">
                <input type="radio">
                  ...
                  <xsl:if test="hideControl != ''">
                    <xsl:attribute name="data-hidden"><xsl:value-of select="hideControl"/></xsl:attribute>
                  </xsl:if>
                  <xsl:if test="removeControl != ''">
                    <xsl:attribute name="data-hidden"><xsl:value-of select="removeControl"/></xsl:attribute>
                  </xsl:if>
                  <xsl:if test="count(changeFieldValues/changeValue) > 0">
                    <xsl:for-each select="changeFieldValues/changeValue">
                      <xsl:attribute name="data-change{position()}"><xsl:value-of select="field"/>|<xsl:value-of select="value"/></xsl:attribute>
                    </xsl:for-each>
                  </xsl:if>
                  <xsl:if test="not($isEditable)">
                    <input type="hidden">
      ...
      <xsl:when test="$node/type='selectUser'">
        ...
        <xsl:if test="$isEditable">
          <img class="btn-open-modal" src="../Content/img/select-user.png" data-toggle="modal" data-target="#selectUserModal">
        ...
        <input type="textbox" readonly="readonly">
          ...
          <xsl:for-each select="$parameters/values/value[changeFieldValues]/..">
            <xsl:for-each select="value/changeFieldValues/changeValue">
              <xsl:attribute name="data-change{position()}"><xsl:value-of select="../../col"/>|<xsl:value-of select="field"/>|<xsl:value-of select="value"/></xsl:attribute>
            </xsl:for-each>
          </xsl:for-each>
        </input>
        <xsl:if test="$isEditable">
          <img class="btn-open-modal" src="../Content/img/select-user.png" data-toggle="modal">
      ...
      <xsl:when test="$node/type='files'">
          ...
          <xsl:for-each select="$attachments/file">
            ...
            <div>
              ...
              <input type="textbox" class="txt-file disabled" readonly="readonly">
                <xsl:attribute name="id"><xsl:value-of select="concat($node/controlId, ':', $position)"/></xsl:attribute>
                <xsl:attribute name="name"><xsl:value-of select="concat($node/controlId, ':', $position)"/></xsl:attribute>
              ...
              <img class="btn-open-file" alt="open" src="../Content/img/download.png">
              ...
              <xsl:if test="$isEditable">
                <img class="btn-delete-file" alt="delete" src="../Content/img/delete.png">
        ...
        <div style="display:none;" class="disabled">
          ...
          <input type="textbox" class="txt-file" readonly="readonly">
            <xsl:attribute name="id"><xsl:value-of select="$node/controlId"/>:<xsl:value-of select="$numberOfAttachments"/></xsl:attribute>
            <xsl:attribute name="name"><xsl:value-of select="$node/controlId"/>:<xsl:value-of select="$numberOfAttachments"/></xsl:attribute>
            <xsl:if test="$isRequired">
              <xsl:attribute name="data-val">true</xsl:attribute>
              <xsl:attribute name="required">required</xsl:attribute>
            </xsl:if>
        ...
        <xsl:if test="$isEditable">
          <img class="btn-file" src="../Content/img/file.png">
      ...
      <xsl:when test="$node/type='list'">
        ...
        <div style="display:none;">
          ...
          <div class="input-list disabled">
            <xsl:attribute name="name"><xsl:value-of select="$node/controlId"/>:0</xsl:attribute>
            <xsl:if test="position() = count($lists/item)">
              <xsl:attribute name="class">input-list last disabled</xsl:attribute>
            </xsl:if>
          </div>
          <input type="hidden" readonly="readonly">
            <xsl:attribute name="id"><xsl:value-of select="$node/controlId"/>:0</xsl:attribute>
            <xsl:attribute name="name"><xsl:value-of select="$node/controlId"/>:0</xsl:attribute>
          </input>
        </div>
      </xsl:when>
                  ...
                  <xsl:choose>
                    <xsl:when test="type='textbox'">
                      <input type="textbox">
                        ...
                        <xsl:attribute name="name"><xsl:value-of select="controlId"/></xsl:attribute>
                        <xsl:if test="$node/value != ''">
                          <xsl:attribute name="value"><xsl:value-of select="$node/value"/></xsl:attribute>
                        </xsl:if>
                        <xsl:if test="not($isEditable)">
      ...
      <xsl:when test="$node/type='customList'">
        <div class="custom-list-field">
          <table class="table">
            ...
            <tbody>
              <tr scope="row">
                <xsl:attribute name="id"><xsl:value-of select="controlId"/>Row:0</xsl:attribute>
                <xsl:attribute name="name"><xsl:value-of select="controlId"/>Row:0</xsl:attribute>
                ...
                <xsl:for-each select="$node/details/field">
                  <td>
                    <xsl:choose>
                      <xsl:when test="type='textbox'">
                        <input type="textbox">
                          <xsl:attribute name="id"><xsl:value-of select="controlId"/>:0</xsl:attribute>
                          <xsl:attribute name="name"><xsl:value-of select="controlId"/>:0</xsl:attribute>
                          <xsl:if test="not($isEditable)">
                            <xsl:attribute name="readonly">readonly</xsl:attribute>
                            <xsl:attribute name="class">disabled</xsl:attribute>
                          </xsl:if>
          ...
          <div class="add-item">
            <a href="#"><xsl:value-of select="$node/addLabel"/></a>
          </div>
        </div>
      </xsl:when>
    </xsl:choose>
  ...
  <xsl:template match="body">
    <link rel="stylesheet" type="text/css" href="../Content/form-request.css"/>
    <script src="../Scripts/form-request.js"/>
    ...
    <div class="container form-container">
      <form id="requestForm">
        <div>
          <input type="hidden">
            ...
            <xsl:attribute name="value"><xsl:value-of select="idForm"/></xsl:attribute>
          </input>
        </div>
        ...
        <div class="tab-content form-request">
          <div class="row header">
            <div class="status">
              Status: <xsl:value-of select="initialInput/status"/>
            </div>
          </div>
              ...
              <div class="container tab-container">
                <xsl:for-each select="../../fields/field[page = $idPage]">
                  ...
                  <xsl:choose>
                      ...
                      <xsl:choose>
                        ...
                        <xsl:otherwise>
                          <div class="row fullrow">
                            ...
                            <div>
                              <xsl:attribute name="class"><xsl:value-of select="$fullrow-length"/></xsl:attribute>
                              <xsl:call-template name="controlForm">
                                <xsl:with-param name="node" select="."/>
                              </xsl:call-template>
                            </div>
                              <xsl:if test="$precedingNode/columnSize != 'full'">
                                <xsl:value-of select="jscript:changePosition()"/>
                              </xsl:if>
                          </div>
                        </xsl:otherwise>
                      </xsl:choose>
                    </xsl:when>
                    <xsl:when test="($isFollowingDisplayed or $isDisplayed) and columnSize='half' and position() mod 2 = jscript:getPosition() and $followingNode/page = page">
                      <div class="row">
                        ...
                        <div class="halfrow">
                          ...
                          <xsl:if test="controlId='Empty'">
                            <xsl:attribute name="class">halfrow empty</xsl:attribute>
                          </xsl:if>
                          <div class="col-sm-2">
                        ...
                        <xsl:if test="$isFollowingDisplayed">
                          <div class="halfrow">
                            <xsl:if test="$followingNode/controlId='Empty'">
                              <xsl:attribute name="class">halfrow empty</xsl:attribute>
                            </xsl:if>
                            <xsl:if test="$followingNode/visible = 'false'">
                              <xsl:attribute name="style">display:none</xsl:attribute>
                            </xsl:if>
                            <div class="col-sm-2">
                              <xsl:if test="$isNextFieldRequired">
                                <div class="required-field">*</div>
                              </xsl:if>
                              <label><xsl:value-of select="$followingNode/label"/></label>
                              <xsl:if test="$followingNode/description!=''">
                                <div class="description">
                                  <xsl:call-template name="split">
                                    <xsl:with-param name="pText" select="$followingNode/description"/>
                                  </xsl:call-template>
                                </div>
                              </xsl:if>
                            </div>
                            <div class="col-sm-4">
                              <xsl:call-template name="controlForm">
                                <xsl:with-param name="node" select="$followingNode"/>
                              </xsl:call-template>
                            </div>
                          </div>
                        </xsl:if>
                    ...
                    <xsl:when test="$isDisplayed and columnSize='half' and ($followingNode/page != page and (not($followingNode) and position() mod 2 = jscript:getPosition()))">
                      <div class="row fullrow">
                        <xsl:if test="visible">
                          <xsl:attribute name="style">display:none</xsl:attribute>
                        </xsl:if>
                      ...
                      <xsl:value-of select="jscript:resetPosition()"/>
                    </xsl:when>
                  </xsl:choose>
                </xsl:for-each>
              </div>
      ...
      <div class="row form-request-footer">
          <div class="status">
              ...
              Created Date: <xsl:value-of select="initialInput/createdDate" />
          </div>
      </div>
    </div>
    <div>
